...
function getParticleOptions(pos, mouseVel) {
...
function getEmitter() {
  ...
  // Har emitter ka apna velocity tracking - interference nahi hoga
  ...
  function update(ctx, mousePos) {
    ...
    for (let i = 0; i < numPerParticlesFrame; i += 1) {
      let options = getParticleOptions(mousePos, mouseVel);
      let particle = getImageParticle(options);
      particles.push(particle);
    }
    while (particles.length > 500) {
      particles.shift(); // Pehla particle delete kar dete hai
    }
  }
